...
### 존재하지 않는 이벤트명 사용 (onSearchEnd 등)
**원인**: IBSheet8은 "Start↔End" 패턴을 사용하지 않음
✗ `onSearchEnd` (존재하지 않는 이벤트)
✓ `onSearchFinish` (올바른 이벤트명)
→ 상세: [events.md](../core/events.md)
...
### 합계행을 Foot 배열로 수동 생성
**원인**: FormulaRow 속성을 모르고 Foot 행을 직접 만들어 계산 시도
✗ `Foot: [{ id:"FOOT1", Kind:"Foot", total: ... }]` (수동 구성)
✓ Col 속성에 `FormulaRow: "Sum"` 추가 (자동 합계행 생성)
