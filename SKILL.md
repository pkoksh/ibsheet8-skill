...
## 🧩 React 환경 레이아웃/마운트 함정 (flex 사이징 / 탭 / 모달)

IBSheetReact 를 React 컨테이너에 끼울 때 가장 흔한 3가지 사고. 상세·코드는 [references/integration/react.md](references/integration/react.md) §4~7.

1. **flex 컨테이너 안 시트는 `minWidth:0` / `minHeight:0` 필수** — `flex:1` 컬럼이나 탭 컨텐츠에 IBSheet(`height:100%`)를 두면 flex item 기본 `min-*:auto` 때문에 내부 min-content 이하로 안 줄어든다. 증상: **"키우면 늘어나는데 줄이면 안 줄어듦"**. `flex:1` 자식에 `minWidth:0`(가로축)/`minHeight:0`(세로축) 추가. (Splitter % 패널·다중 컬럼 마지막 flex 컬럼도 동일)
2. **`display:none` 컨테이너에서 첫 마운트되면 size-0 고정** — 비활성 탭에 시트를 숨겨 마운트하면 0 사이즈로 그려지고 탭을 열어도 회복 안 됨. ① 활성된 적 있는 탭만 mount(lazy mount `Set`) 하거나 ② `keepMounted` + 탭 전환 시 `requestAnimationFrame(() => sheet.resize())`. 그리드 `Events` 핸들러의 탭 분기는 state 대신 `ref` 로 읽는다(생성 시점 stale closure).
3. **모달(`DialogContent`) 안 시트는 `maxWidth:'none'` + `height`/`maxHeight`/`overflow:hidden` 이중 잠금** — 안 하면 512px 로 잘리거나, 시트 동적 높이가 grid auto-row 로 팝업 밖까지 노출. 시트 wrapper 에도 `overflow:hidden` 한 번 더.

> 디버깅: 브라우저 콘솔 `window.GetSheets()[0]` 로 생성된 시트 인스턴스 접근 → `getDataRows()` / `getValue(row, col)` / `DataRowCount` 로 실 적재 데이터·숨김 컬럼 즉석 확인.
...
| Tree Grid | [references/features/tree-grid.md](references/features/tree-grid.md) | Hierarchical data + **React 패턴(showTreeLevel race·`onSearchFinish` 예약·`setExpandRow`(Expanded readonly)·좌우 트리 동기화·lvl 색상·DnD 국소 재번호)** |
...
| Topic | File | Description |
|-------|------|-------------|
| React | [references/integration/react.md](references/integration/react.md) | React componentization (loader/IBSheetReact/TypeScript) + **flex 사이징(`minWidth/minHeight:0`)·탭 마운트(size-0)·모달 크기·`GetSheets()` 디버깅** |
