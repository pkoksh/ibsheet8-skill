var IBSMSG = IBSMSG || {};

IBSMSG.En = {
  "Lang": {
    "Create": {
      "MultiRecordDiffColLength": "The number of columns in the %2nd unit data row of '%1' is different. (Expected: %3, Actual: %4)",
      "NoCanUseMultiRecordWithTree": "The MultiRecord feature cannot be used together with the tree feature (MainCol).",
      "TwoDimensionalArray": "The MultiRecord feature must be configured as a two-dimensional array.",
      "MultiRecordNameErr": "The 'Name' property must be set. (Missing Name property in unit data row %1, column %2)",
      "MustBeArray": "'%1' must be an array."
    },
    "Alert": {
      "CanReloadStart": "This command ",
      "CanReloadChanges": "updates all changes to server",
      "CanCancelChanges": "cancels all changes",
      "And": " and ",
      "CanReloadSelect": "clears all selection",
      "CanReloadEnd": "! Do you want to continue?",
      "ErrTimeout": "Cannot download data, timeout expired",
      "AskTimeout": "Cannot download data, server timeout !\nDo you want to repeat request ?",
      "UploadTimeout": "Cannot upload data, timeout expired",
      "AskUploadTimeout": "Cannot upload data, server timeout !\nDo you want to send data again ?",
      "ErrHide": "You cannot hide all variable columns!",
      "ErrHideExt": "Width of all fixed columns is too wide!",
      "ErrPrintOpen": "The print window could not be opened\n\n Permit popup windows to let print the sheet",
      "ErrPrint": "The document was not successfully printed",
      "ErrCheck": "Synchronizing with server failed!\nDo you want to temporary disable the checking for updates?",
      "SearchHelp": "Enter string to search for like in Google\n\nindividual keywords separate by space\nphrases place into double quotes\nvariants separate by uppercase OR keyword\nexceptions precede by \"-\" (minus)\n\nOr enter expression to search by\n\nspecify columns caption to compare\nuse operators < <= > >= = <> ( ) + - * /\nuse AND OR keywords to join expressions\nuse special keywords STARTS ENDS CONTAINS\n   for comparing strings (NOT to negate)\nexample: customer starts martin and date <= 1/1/2006",
      "NotFound": "Nothing found",
      "SearchStart": "Nothing found\nDo you want to continue from beginning?",
      "SearchError": "The search expression is invalid !",
      "SearchNotFound": "No matching search results found.",
      "Invalid": "Invalid value",
      "DelRow": "Are you sure to delete row '%d' ?",
      "DelSelected": "Are you sure to delete %d selected rows ?",
      "StyleErr": "Cannot load IBSheet CSS style !",
      "ExportDownload": "Download",
      "FoundResults": "Found %d results",
      "PrintPrepared": "Sheet is ready to print",
      "PrintReady": "Sheet is ready to print\nChoose Print Preview option in menu to update page size\nOr choose Print option or press Ctrl+P to print directly",
      "PrintCloseWindow": "\nAfter print is finished, you can close this page",
      "PrintCloseTag": "\nAfter print is finished, you can click to the sheet to return",
      "PivotReload": "Do you want to re-create the pivot sheet?",
      "FormulaCircularAsk": "Formula contains circular cell reference\nDo you want to permit it?",
      "DelCol": "Are you sure to delete column '%d' ?",
      "DelSelectedCols": "Are you sure to delete %d selected columns ?",
      "SearchEnd": "Nothing found\nDo you want to continue from end?",
      "ExportIE": "Not available in IE 9 and older",
      "ExportError": "Export failed!",
      "DefaultSheet": "sheet",
      "NewSheet": "sheet%1",
      "LoadSheetErrorMissingModule": "Loading data failed because of missing IBSheet module %1",
      "StripHtml": "The change will strip formatting from the text!&#x0a;Do you want to save the change?",
      "NoRow": "%1 does not exist.",
      "AlreadyAttrSame": "Already set to the same value as %1",
      "ToManyFixedCols": "The parameter is greater than the number of columns to be fixed.",
      "ToManyFixedRows": "The parameter is greater than the number of rows to be fixed.",
      "NotWorking": "%2 can not be used in %1",
      "ErrValueLimit": "Out of number value in javascript. <br/>(%2 ~ %1)",
      "InvalidExcelFileExtension": "Cancel action because not allow file format(ex xlsx, xls).",
      "InvalidTextFileExtension": "Cancel action because not allow file format(ex txt, csv).",
      "LoadExcelError": "Throw exception when file loading.",
      "ExceedFileSize": "Maximum file size over! [Maximum size : %d]",
      "AlertError": "IBSheet communication error \r\n\r\nURL: ",
      "ResultErrEmptyURL": "<span style='color: #ed2f5f; font-weight:bold;'>Empty url value.</span>",
      "ResultErrBadURL": "<span style='color: #ed2f5f; font-weight:bold;'>Bad url setting.</span>",
      "ResultErrNotFound": "<span style='color: #ed2f5f; font-weight:bold;'>Not found url.</span>",
      "ResultErrBadDocumentFormat": "<span style='color: #ed2f5f; font-weight:bold;'>Bad documentation format.</span>",
      "ResultErrEmptyResponse": "<span style='color: #ed2f5f; font-weight:bold;'>The response result is empty.</span>",
      "ResultErrRequestTimeout": "<span style='color: #ed2f5f; font-weight:bold;'>Request timeout.</span>",
      "ResultErrBadDataFormat": "<span style='color: #ed2f5f; font-weight:bold;'>Bad data format.</span>",
      "ResultErrUnknown": "<span style='color: #ed2f5f; font-weight:bold;'>Unknown Error!!</span>",
      "ApiError": "An error occurred in function '%1'.",
      "RemoveNotWorking": "%2 row cannot be deleted in %1.",
      "EditNotWorking": "Cells cannot be edited",
      "ExportDataErrMsg": "It cannot find the JSZip module of IBSheet for downloading the data.",
      "ImportDataErrMsg": "It cannot find the JSZip module of IBSheet for uploading the data.",
      "NoXlsxFullMinJS": "xls file format is not supported.",
      "CookieNotFound": "Cookie not found. Please make sure cookies are enabled in your browser.",
      "TabNameEmpty": "The name of the workSheet you entered is invalid.",
      "EmptyInfoErr": "Please enter sheet information.",
      "InputInfoErr": "The entered sheet information is incorrect.",
      "NoFilterDataForPivot": "None of the filter data exists to create the pivot sheet.",
      "NoExcelPlugin": "To use XlsImportMode, you must include the Excel plug-in."
    },
    "Text": {
      "DelSelected": "Deleting selected rows",
      "ExtentErr": "Too small sheet size",
      "Sort": "Sorting rows ...",
      "SelectAll": "Changing selection of all rows",
      "DoFilter": "Filtering rows ...",
      "UpdateSheet": "Updating view ...",
      "CollapseAll": "Collapsing all rows ...",
      "ExpandAll": "Expanding all rows ...",
      "Render": "Rendering ...",
      "Page": "Page",
      "NoPages": "Empty",
      "UpdateCfg": "Updating settings ...",
      "StartErr": "Fatal error ! <br/>IBSheet cannot render",
      "Calculate": "Calculating cells ...",
      "UpdateValues": "Updating values ...",
      "UpdateTree": "Updating tree ...",
      "PageErr": "Cannot download this data part !",
      "Layout": "Loading layout ...",
      "Load": "Loading data ...",
      "ColumnsCaption": "Select columns to display",
      "ColUpdate": "Updating columns ...",
      "Picture": "Picture",
      "DefaultsDate": "Select date ...",
      "DefaultsButton": "Select ...",
      "GroupCustom": "To group by, drag column caption here ...",
      "Group": "Grouping rows ...",
      "DefaultsFilterOff": "(All)",
      "Items": "Items %d - %d",
      "Print": "<h2><center>Please wait while generating report for printing ...</center></h2>",
      "SearchMethodList": "|Automatic|Search&#160;like&#160;Google|Search&#160;by&#160;expression",
      "Contains": "contains,has",
      "Starts": "starts,starts with,starts by,begins,begins with,begins by",
      "Ends": "ends,ends by, ends with",
      "And": "and",
      "Or": "or",
      "Not": "not",
      "SearchSearch": "Search",
      "SearchFilter": "Filter",
      "SearchSelect": "Select",
      "SearchMark": "Mark",
      "SearchFind": "Find",
      "SearchClear": "Clear",
      "SearchHelp": "Help",
      "Search": "Searching ...",
      "Printed": "Please switch to window containing the report to print it",
      "DoUndo": "Performing undo ...",
      "DoRedo": "Performing redo ...",
      "LoadStyles": "Loading style ...",
      "SetStyle": "Updating style ...",
      "LoadPage": "Loading",
      "RenderPage": "Rendering",
      "ColWidth": "Changing width of column '%d'",
      "ColMove": "Moving column '%d'",
      "Password": "***",
      "DefaultsNone": "Clear all",
      "RadioFilterOff": "<button style='white-space: nowrap;'>Clear</button>",
      "DragObjectMove": "moving row <b>%d</b>",
      "DragObjectCopy": "copying row <b>%d</b>",
      "DragObjectMoreMove": "moving <b'>%d</b> rows",
      "DragObjectMoreCopy": "copying <b>%d</b> rows",
      "ExportFinished": "<center><b>Report generated</b><br/><br/>Click button for download<br/></center><br/>",
      "RenderProgressText": "Finished %pos pages from %cnt",
      "RenderProgressCancel": "Render on background",
      "PrintProgressCaption": "Generating report",
      "PrintProgressText": "Finished %pos rows from %cnt",
      "PrintProgressCancel": "Cancel report",
      "ExportProgressCaption": "Generating report",
      "ExportProgressText": "Finished %pos rows from %cnt",
      "ExportProgressCancel": "Cancel report",
      "ExpandProgressCaption": "Expanding all rows",
      "ExpandProgressText": "Finished %pos rows from %pos",
      "ExpandProgressCancel": "Stop expanding",
      "ExportCaption": "Select columns to export",
      "PrintCaption": "Select columns to print",
      "DefaultsAll": "Select all",
      "DefaultsAlphabet": "%d ...",
      "RemoveUnused": "Clearing unused pages",
      "ErrorSave": "Saving changes on server failed!",
      "DatesRepeat": "Repeat",
      "DatesStart": "Start",
      "DatesEndTime": "End",
      "DatesValue": "Value",
      "DatesRepeatEnum": "||Weekly|Daily|Hourly",
      "DatesRepeatKeys": "||w|d|h",
      "RenderProgressCaptionRows": "Rendering row pages",
      "RenderProgressCaptionCols": "Rendering column pages",
      "RenderProgressCaptionChildren": "Rendering tree pages",
      "CalendarNone": "<s>None</s>",
      "CalendarEmpty": "<s>Default</s>",
      "CalendarEdit": "<s>Edit ...</s>",
      "ExpandCols": "Expanding columns",
      "CollapseCols": "Collapsing columns",
      "PagingUpdate": "Creating pages ...",
      "CopySlow": "The copy action took too long time (%d ms), please press the Ctrl+C again to finish the action",
      "CopyOk": "The copy action finished successfuly",
      "LevelsTip": "Expand rows up to %d level",
      "FileIE": "Not available in IE<=9",
      "Paste": "Pasting data from clipboard ...",
      "FormulaError1": "Formula <b>%1</b> cannot be parsed",
      "FormulaError2": "Incorrect cell reference in formula <b>%1</b>",
      "FormulaError3": "Incorrect item <b>%2</b> in formula <b>%1</b>",
      "FormulaError4": "Incorrect formula <b>%1</b> with error <i>%2</i>",
      "FormulaError5": "Formula cannot be calculated with error <i>%2</i>",
      "FormulaError6": "Formula <b>%1</b> returned <b>null</b> result",
      "FormulaError7": "Formula <b>%1</b> returned <b>NaN</b> result",
      "NotInList": "The value <b>%1</b> is not in the item list",
      "EmptyValue": "empty value",
      "CannotEdit": "Not editable value",
      "NoStyle": "Style cannot be changed",
      "NoFormat": "Format cannot be changed",
      "NoNumber": "Cell value is not a number",
      "EditMaskError": "The column %2 cell in row %1 contains non-inputable data.",
      "FormulaRestrict": "Formula <b>%1</b> is restricted with error <i>%2</i>",
      "SizeChanged": "The value <b>%1</b> changed due size limit",
      "ResultText": "The value <b>%1</b> is invalid",
      "EditErrorsMessage": "%1",
      "EditChangesMessage": "%1",
      "EditErrorsMessageCell": "%1 in <b>%2</b><br>",
      "EditChangesMessageCell": "%1 in <b>%2</b><br>",
      "EditErrorsMessageMore": "There are <b>%1</b> more problems ...",
      "FormulaCircular": "Formula contains circular cell reference",
      "FormulaCircularAlert": "Formula contains circular cell reference",
      "ErrAdd": "Cannot add new row to parent %1",
      "ErrAddRoot": "Cannot add new row to root",
      "DelSelectedCols": "Deleting selected columns",
      "SearchFindPrev": "Prev",
      "NoMenu": "No action available",
      "SetStyleSame": "No cell was changed",
      "SetStyleError": "No cell was changed due edit restrictions",
      "SetStyleErrors": "There were not changed %1 cells due edit restrictions",
      "PagerSep1": "\n|\n",
      "PagerSep2": "\n=>\n",
      "PagerSep3": ",\n",
      "PagerNone": "...",
      "LoadSheet": "Reading sheet ...",
      "DiscardSheet": "Do you want to discard all your data and load the data from file <b>%1</b>?",
      "AddSheet": "Do you want to add the data from file <b>%1</b> to actual data?",
      "OpenSheet": "Do you want to add the data from file <b>%1</b> to actual data<br>or discard all your data and load the data from file <b>%1</b>?",
      "LoadSheetError": "Loading data from <b>%1</b> failed with error <b>%2</b>",
      "ParseSheetError": "Parsing data from <b>%1</b> failed",
      "SheetsUnsupported": "Opening xlsx files is not supported in your browser",
      "DeleteSheet": "Do you want delete sheet <b>%1</b>?",
      "DeleteTab": "Do you want delete tab <b>%1</b>?",
      "OnlyXlsx": "Cannot open <b>%1</b>. Only <b>xlsx</b> files are supported",
      "CompatibleStyles": "Compatible",
      "ShrinkStyle": "Too large style, shrinking style size",
      "SetSize": "Changing style size",
      "UniqueSheet": " (%1)",
      "ShrinkSize": "There is not enough space to display the sheet fully<br>Do you want to shrink the sheet style size?",
      "EnterFormat": "Enter format string like in MS Excel",
      "FormulaError8": "External reference to unknown sheet <b>%2</b> in formula <b>%1</b>.",
      "FormulaError9": "Not supported function <b>%2</b> in formula <b>%1</b>",
      "FormulaError10": "Not supported reference to external file <b>%2</b> in formula <b>%1</b>",
      "FormulaError11": "Not yet implemented array formula or unexpected cell range <b>%2</b> in formula <b>%1</b>",
      "MoveRows": "Moving %d rows",
      "NoSave": "No changes on sheet",
      "ReqStatusDeleted": "Deleted",
      "ReqStatusAdded": "Added",
      "ReqStatusChanged": "Changed",
      "ReqStatusEmpty": "",
      "TreeLevelName": "tLELVEL",
      "ConfirmSaveDataRows": "<div><div>Added: %1case</div><div>Changed: %2case</div><div>Deleted: %3case</div> Do you want save?</div>",
      "ConfirmSaveDataRows1": "<div><div>Added: %1case</div><div>Changed: %2case</div> Do you want save?</div>",
      "ConfirmSaveDataRows2": "<div><div>Total: %1case</div> Do you want save?</div>",
      "ConfirmSaveDataRows3": "<div><div>Added: %1case</div><div>Changed: %2case</div><div>Deleted: %3case</div><div>Moved: %4case</div> Do you want save?</div>",
      "RequiredError": "%1 row's %2 column cell is required fields.",
      "SubSumText": "Subtotal",
      "CumulateSumText": "Total",
      "PivotText": "Pivot",
      "AcceptPivotRows": "Pivot Row:",
      "AcceptPivotCols": "Pivot Column:",
      "AcceptPivotData": "Pivot Data:",
      "CreatePivotSheet": "Create Pivot",
      "ShowPivotSheet": "Show Pivot",
      "SwitchPivotSheet": "Origin Sheet",
      "ClearPivotSheet": "Delete Pivot",
      "ShowAllSheets": "View together",
      "CreatePivotError": "There is something wrong in option!",
      "ShowPivotError": "You have to create pivot sheet in advance!",
      "PivotCriteria": "Criteria",
      "PivotTotal": "SUM",
      "PivotMax": "Max",
      "PivotMin": "Min",
      "PivotSum": "SUM",
      "PivotCount": "Count",
      "PivotAvg": "Avg",
      "PivotCreating": "Pivot Creating...",
      "DateInCorrectError": "Incorrect Date Value",
      "NoSearchData": "No Data Found",
      "SearchDesc": "Data Searching...",
      "SaveDesc": "Data Saving...",
      "ExportFile": "File Exporting...",
      "UploadFile": "File Uploading...",
      "ProcessDesc": "Processing...",
      "RowMessageCaption": "%1 row",
      "ColMessageCaption": "%1 column",
      "FilterEmptyValue": "No field value",
      "SummaryCount": "Cells",
      "SummaryTotal": "Total Value",
      "SummaryAverage": "Average Value",
      "KoreanExp": "[ㄱ-ㅎㅏ-ㅣ가-힣]",
      "ExceptUnicodeExp": "[ㄱ-ㅎㅏ-ㅣ가-힣a-zA-Z]",
      "InvaildJSONParse": "Invalid data structure, parsing error",
      "ErrorSearch": "Searching changes on server failed!",
      "SearchProgressMessage": "Data Searching...",
      "DataSearchingMessage": [
        "API call and Response Waiting",
        "Parse data",
        "Render Start",
        "Render Finished"
      ],
      "PageLengthMessage": "Select the number to display on the sheet",
      "Filter_13": {
        "Up": "Top",
        "Down": "Bottom",
        "Item": "Items",
        "Percent": "%",
        "Success": "Ok",
        "Cancel": "Cancel"
      },
      "FilterTip": {
        "Date": [
          "Date Filtering Method",
          "1. Filtering Specific Dates",
          "Put year, month, day without and separators. ex) '20240125'",
          "2. Filtering Specific Periods",
          "Put date-to-date format without any spaces. ex) '20240101~20240131'"
        ],
        "Number": [
          "Number Filtering Method",
          "1. Common Filtering",
          "Put numbers, then click Icon to select filtering options.",
          "2. Section Filtering",
          "Put numbers to filter connected by ~ without any spaces. ex) '100~4000'"
        ],
        "Text": [
          "Text Filtering Method",
          "1. Common Filtering",
          "Put texts, then click Icon to select filtering options.",
          "2. 'or' Filtering",
          "Put texts to filter connected by ; without any spaces. ex) 'thomas;jake'",
          "3. and Filtering",
          "Put texts to filter connected by , without any spaces. ex) 'thomas,jake'"
        ]
      },
      "StatusLabelChange": "The value in row cell $2 of column $1 has been modified from $3 to $4.",
      "StatusLabelAllChecked": "All checked boxes became $1.",
      "StatusLabelDataLoaded": "$1 worth of data has been loaded.",
      "StatusLabelValueUndo": "The value in the $2 row cell of column $1 has been undone.",
      "StatusLabelSearch": "Search ",
      "StatusLabelExcel": "Excel ",
      "StatusLabelFileUpload": "The file in row cell $2 of column $1 has been uploaded to $3.",
      "StatusLabelFileDelete": "The file in the $2 row cell of column $1 has been removed.",
      "StatusLabelColMove": "Column $1 has been moved to $3 in column $2.",
      "StatusLabelRowMove": "Row $1 has been moved to $3 in row $2.",
      "StatusLabelSort": "Column $1 is sorted by $2.",
      "StatusLabelSortClear": "Sorting has been reset.",
      "StatusLabelFilter": "Column $1 has been filtered by $2.",
      "StatusLabelFilterClear": "Filtering has been reset.",
      "StatusLabelNullValue": "empty value",
      "StatusLabelMovingUp": "topside",
      "StatusLabelMovingDown": "down",
      "StatusLabelSameLevel": "lower level row",
      "FilterDialog": {
        "TextFilter": "Text Filter",
        "NumberFilter": "Number Filter",
        "DateFilter": "Date Filter",
        "EmptyValue": "(Empty Value)",
        "TextFilterPlaceHolder": "Section Filters can be set by ~"
      },
      "ValidationError": {
        "Required": "%1 row's %2 column cell is required fields.",
        "Size": "The number of characters entered in column %2 of row %1 is greater than the number of characters you can enter.",
        "Mask": "%2 column in row %1 did not enter the correct value for masking."
      },
      "ScaleSmallerThan": "Scaling below %d is not allowed.",
      "StyleRow": {
        "Header": "Header",
        "Row": "Data",
        "Alternate": "Alternate",
        "Save": "Save",
        "Load": "Load",
        "ApplyAll": "Apply to All Sheets",
        "Initialize": "Initialize",
        "TextSize": "Font Size",
        "TextColor": "Font Color",
        "BackgroundColor": "Background Color",
        "Theme": "Theme",
        "SaveSuccess": "Style saved successfully.",
        "SaveFail": "Failed to save style.",
        "NoStyleDataToSave": "There is no style data to save.",
        "LoadFail": "Failed to load style.",
        "NoStyleDataToLoad": "There is no style data to load.",
        "NoServerUrl": "Server URL is not set.",
        "MaxFontSize": "Font size cannot be larger than %1px.",
        "MinFontSize": "Font size cannot be smaller than %1px.",
        "InitAll": "Initialize All Sheets",
        "InitSuccess": "Style initialized successfully.",
        "InitFail": "Failed to initialize style.",
        "DeleteFail": "Failed to delete style information."
      },
      "SizeError": "The number of characters in column %1 row %2 has exceeded the number of characters you can enter.",
      "ResultMaskError": "The column %2 cell in row %1 contains non-inputable data."
    },
    "Memo": {
      "Head": "Memo",
      "Delete": "Delete"
    },
    "MenuButtons": {
      "Ok": "OK",
      "Clear": "Clear",
      "Today": "Today",
      "All": "All on",
      "Cancel": "Cancel",
      "Close": "Close",
      "Yesterday": "Yesterday",
      "EmptyTip": "Empty date",
      "Yes": "Yes",
      "No": "No",
      "Always": "Always",
      "Never": "Never",
      "ShowAll": "Show all",
      "HideAll": "Hide all",
      "Reset": "Defaults",
      "Add": "Add",
      "Discard": "Discard",
      "Once": "Once",
      "Shrink": "Shrink style size",
      "Month": "this month",
      "Apply": "Apply"
    },
    "TimePicker": {
      "Head": "Time Picker"
    },
    "MenuCell": {
      "CopyRow": "Duplicate the row",
      "CopyRowBelow": "Duplicate the row",
      "CopyTree": "Duplicate the row tree",
      "CopyTreeBelow": "Duplicate the row tree",
      "CopyEmpty": "Add empty row tree above",
      "CopyEmptyBelow": "Add empty row tree below",
      "CopySelected": "Copy %1 selected rows above",
      "CopySelectedBelow": "Copy %1 selected rows below",
      "CopySelectedTree": "Copy %1 selected row trees above",
      "CopySelectedTreeBelow": "Copy %1 selected row trees below",
      "CopySelectedEmpty": "Add %1 selected empty row trees above",
      "CopySelectedEmptyBelow": "Add %1 selected empty row trees below",
      "CopySelectedChild": "Copy %1 selected rows as the first children",
      "CopySelectedTreeChild": "Copy %1 selected row trees as the first children",
      "CopySelectedEmptyChild": "Add %1 selected empty row trees as the first children",
      "CopySelectedChildEnd": "Copy %1 selected rows as the last children",
      "CopySelectedTreeChildEnd": "Copy %1 selected row trees as the last children",
      "CopySelectedEmptyChildEnd": "Add %1 selected empty row trees as the last children",
      "CopySelectedEnd": "Copy %1 selected rows to the end",
      "CopySelectedEndPage": "Copy %1 selected rows to the end of page",
      "CopySelectedEndSheet": "Copy %1 selected rows to the end of sheet",
      "CopySelectedTreeEnd": "Copy %1 selected row trees to the end",
      "CopySelectedTreeEndPage": "Copy %1 selected row trees to the end of page",
      "CopySelectedTreeEndSheet": "Copy %1 selected row trees to the end of sheet",
      "CopySelectedEmptyEnd": "Add %1 selected empty row trees to the end",
      "CopySelectedEmptyEndPage": "Add %1 selected empty row trees to the end of page",
      "CopySelectedEmptyEndSheet": "Add %1 selected empty row trees to the end of sheet",
      "AddRow": "Add new empty row above",
      "AddChild": "Add new empty row as the first child",
      "AddChildEnd": "Add new empty row as the last child",
      "AddRowBelow": "Add new empty row below",
      "AddRowEnd": "Add new empty row to the end",
      "AddRowEndPage": "Add new empty row to the end of page",
      "AddRowEndSheet": "Add new empty row to the end of sheet",
      "CopyRows": "Copy %1 focused rows above",
      "CopyRowsBelow": "Copy %1 focused rows below",
      "CopyRowsTree": "Copy %1 focused row trees above",
      "CopyRowsTreeBelow": "Copy %1 focused row trees below",
      "CopyRowsEmpty": "Add %1 empty focused row trees above",
      "CopyRowsEmptyBelow": "Add %1 empty focused row trees below",
      "AddRows": "Add %1 empty focused rows above",
      "AddRowsBelow": "Add %1 empty focused rows below",
      "AddSelected": "Add %1 empty selected rows above",
      "AddSelectedBelow": "Add %1 empty selected rows below",
      "DeleteRow": "Delete row",
      "UndeleteRow": "Undelete row",
      "RemoveRow": "Remove row",
      "DeleteRows": "Delete %1 focused rows",
      "UndeleteRows": "Undelete %1 focused rows",
      "RemoveRows": "Remove %1 focused rows",
      "DeleteSelected": "Delete %1 selected rows",
      "UndeleteSelected": "Undelete %1 selected rows",
      "RemoveSelected": "Remove %1 selected rows",
      "CopyCol": "Duplicate the column",
      "CopyColNext": "Duplicate the column",
      "CopyCols": "Copy %1 focused columns left",
      "CopyColsNext": "Copy %1 focused columns right",
      "CopySelectedCols": "Copy %1 selected columns left",
      "CopySelectedColsNext": "Copy %1 selected columns right",
      "AddCol": "Add new empty column left",
      "AddColNext": "Add new empty column right",
      "AddCols": "Add %1 empty focused columns left",
      "AddColsNext": "Add %1 empty focused columns right",
      "AddSelectedCols": "Add %1 empty selected columns left",
      "AddSelectedColsNext": "Add %1 empty selected columns right",
      "AddColEnd": "Add new empty column to the end",
      "DeleteCol": "Delete column",
      "UndeleteCol": "Undelete column",
      "RemoveCol": "Remove column",
      "DeleteCols": "Delete %1 focused columns",
      "UndeleteCols": "Undelete %1 focused columns",
      "RemoveCols": "Remove %1 focused columns",
      "DeleteSelectedCols": "Delete %1 selected columns",
      "UndeleteSelectedCols": "Undelete %1 selected columns",
      "RemoveSelectedCols": "Remove %1 selected columns",
      "ClearSelection": "Unselect all %1 rows and columns",
      "SelectRow": "Select row",
      "DeselectRow": "Unselect row",
      "SelectFocusedRows": "Select %1 focused rows",
      "DeselectFocusedRows": "Unselect %1 focused rows",
      "SelectAll": "Select all %1 rows",
      "DeselectAll": "Unselect all %1 rows",
      "InvertAll": "Invert selection of all %1 rows",
      "SelectCol": "Select column",
      "DeselectCol": "Unselect column",
      "SelectFocusedCols": "Select %1 focused columns",
      "DeselectFocusedCols": "Unselect %1 focused columns",
      "SelectCell": "Select cell",
      "DeselectCell": "Unselect cell",
      "SelectRowRange": "Select %1 rows in the area",
      "DeselectRowRange": "Unselect %1 rows in the area",
      "SelectColRange": "Select %1 columns in the area",
      "DeselectColRange": "Unselect %1 columns in the area",
      "SelectCellRange": "Select %1 cells in the area",
      "DeselectCellRange": "Unselect %1 cells in the area",
      "SelectColCells": "Select all %1 cells in column",
      "DeselectColCells": "Unselect all %1 cells in column",
      "SelectRowCells": "Select all %1 cells in row",
      "DeselectRowCells": "Unselect all %1 cells in row",
      "SelectAllCells": "Select all %1 cells",
      "DeselectAllCells": "Unselect all %1 cells",
      "SelectAllCols": "Select all %1 columns",
      "DeselectAllCols": "Unselect all %1 columns",
      "SpanSelected": "Span %1 selected cell ranges",
      "SpanCells": "Span %1 cells",
      "SplitSelected": "Split %1 selected spanned cells",
      "SplitCells": "Split spanned %1 cells",
      "SplitCell": "Split spanned cell",
      "Expand": "Expand row",
      "Collapse": "Collapse row",
      "ExpandAll": "Expand all %1 rows",
      "CollapseAll": "Collapse all %1 rows",
      "CollapseAllRoot": "Collapse all %1 root rows",
      "Outdent": "Outdent row",
      "Indent": "Indent row",
      "OutdentRows": "Outdent %1 rows",
      "IndentRows": "Indent %1 rows",
      "OutdentSelected": "Outdent %1 selected rows",
      "IndentSelected": "Indent %1 selected rows",
      "ExpandCell": "Expand or collapse cell",
      "ExpandCol": "Expand column",
      "CollapseCol": "Collapse column",
      "ExpandRow": "Expand row",
      "CollapseRow": "Collapse row",
      "ExpandAllCells": "Expand all columns",
      "CollapseAllCells": "Collapse all columns",
      "ExpandRowCells": "Expand all columns in row",
      "CollapseRowCells": "Collapse all columns in row",
      "HideRow": "Hide row",
      "HideRows": "Hide %1 focused rows",
      "HideSelectedRows": "Hide %1 selected rows",
      "ShowRowAbove": "Show hidden row above",
      "ShowRowBelow": "Show hidden row below",
      "ShowAllRows": "Show all %1 hidden rows",
      "ShowRows": "Show %1 hidden rows",
      "HideCol": "Hide column",
      "HideCols": "Hide %1 focused columns",
      "HideSelectedCols": "Hide %1 selected columns",
      "ShowColLeft": "Show hidden column on the left",
      "ShowColRight": "Show hidden column on the right",
      "ShowAllCols": "Show all %1 hidden columns",
      "ShowCols": "Show %1 hidden columns",
      "SearchOn": "Enable search",
      "SearchOff": "Disable search",
      "SortAsc": "Sort by the column ascending",
      "SortDesc": "Sort by the column descending",
      "SortAscOne": "Sort by the one column ascending",
      "SortDescOne": "Sort by the one column descending",
      "SortAscAppend": "Sort by the next column ascending",
      "SortDescAppend": "Sort by the next column descending",
      "NoSort": "Do not sort by the column",
      "DefaultSort": "Restore original sorting",
      "SortOn": "Enable sorting",
      "SortOff": "Disable sorting",
      "Undo": "Undo last action",
      "UndoAll": "Undo all %1 actions",
      "Redo": "Redo last undone action",
      "RedoAll": "Redo all %1 undone actions",
      "ClearUndo": "Clear undo buffer",
      "Print": "Print the sheet",
      "ExportPDF": "Print the sheet to PDF",
      "Export": "Export the sheet to XLSX, XLS or CSV",
      "Repaint": "Repaint, clear unused pages",
      "RenderPages": "Change the paging",
      "PagingOn": "Enable paging, clear unused pages",
      "PagingOff": "Disable paging, render all pages",
      "ShowCfg": "Show configuration menu",
      "ShowColumns": "Show columns menu",
      "ShowDebug": "Show debug window",
      "Reload": "Reload sheet, cancel changes",
      "Validate": "Check the data for errors",
      "Save": "Save changes to server",
      "GoUp": "Focus up",
      "GoUpEdit": "Focus up",
      "GoDown": "Focus down",
      "GoDownEdit": "Focus down",
      "GoDownAdd": "Focus down",
      "GoDownEditAdd": "Focus down",
      "GoLeft": "Focus left",
      "GoLeftEdit": "Focus left",
      "GoRight": "Focus right",
      "GoRightEdit": "Focus right",
      "GoFirst": "Focus first row",
      "GoLast": "Focus last row",
      "PageDown": "Go to next page",
      "PageUp": "Go to previous page",
      "PageDownFull": "Go to next page",
      "PageUpFull": "Go to previous page",
      "ShowDetail": "Show the row detail",
      "RefreshDetail": "Show the row detail",
      "ClearDetail": "Clear the detail",
      "ShowHintStatic": "Show full cell content",
      "SheetResizeDefault": "Restore sheet size",
      "GroupBy": "Group by the column",
      "GroupByLast": "Add the column to grouping",
      "UngroupBy": "Ungroup by the column",
      "GroupOn": "Enable grouping",
      "GroupOff": "Disable grouping",
      "Focus": "Focus cell",
      "FocusEdit": "Focus and edit cell",
      "FocusAndEdit": "Focus and edit cell",
      "ChangeFocus": "Focus cell",
      "ChangeFocusRow": "Focus row cell",
      "ChangeFocusCol": "Focus column cell",
      "Blur": "Remove focus",
      "BlurFocused": "Remove focus",
      "FocusWholeRow": "Focus all cells in row",
      "FocusWholeCol": "Focus all cells in column",
      "FocusWholeSheet": "Focus all cells in sheet",
      "StartEdit": "Edit focused cell",
      "StartEditEmpty": "Edit focused cell empty",
      "StartEditCell": "Edit the cell",
      "StartEditCellEmpty": "Edit the cell empty",
      "CancelEdit": "Cancel editing",
      "AcceptEdit": "Finish editing",
      "ShowCalendar": "Pick up date from calendar",
      "ShowDates": "Pick up dates from table",
      "ShowLink": "Navigate to url",
      "Button": "Press the button",
      "ButtonClick": "Press the right button",
      "IconClick": "Press the left button",
      "ShowHelp": "Show help",
      "ShowFile": "Pick up a file",
      "ShowDefaults": "Show predefined items",
      "ShowDefaultsMenu": "Show predefined items",
      "ChangeBool": "Change checkbox",
      "CheckBool": "Check the icon",
      "UncheckBool": "Uncheck the icon",
      "ClearBool": "Clear the cell",
      "ChangeRadioLeft": "Cycle radio left",
      "ChangeRadioLeftCycle": "Cycle radio left",
      "ChangeRadioRight": "Cycle radio right",
      "ChangeRadioRightCycle": "Cycle radio right",
      "CheckIcon": "Check the icon",
      "UncheckIcon": "Uncheck the icon",
      "SetChecked": "Change checkbox",
      "ShowFilterMenu": "Show filter menu",
      "ShowFilterMenuRow": "Show filter menu",
      "FilterBy": "Filter by the cell value",
      "FilterByMenu": "Choose filter for the cell value",
      "FilterByMenuRow": "Choose filter for the cell value",
      "ClearFilter": "Clear filter for the column",
      "ClearFilters": "Clear all filters",
      "FilterOn": "Enable filtering",
      "FilterOff": "Disable filtering",
      "ClearCell": "Clear the cell",
      "ClearValue": "Clear the cell value",
      "ClearStyle": "Clear the cell style",
      "ClearCells": "Clear all %1 focused cells",
      "ClearValues": "Clear all focused cell values",
      "ClearStyles": "Clear all focused cell styles",
      "ClearSelectedCells": "Clear all selected cells",
      "ClearSelectedValues": "Clear all selected cell values",
      "ClearSelectedStyles": "Clear all selected cell styles",
      "ClearCol": "Clear the column",
      "ClearColValues": "Clear the column values",
      "ClearColStyles": "Clear the column styles",
      "ClearSelectedCols": "Clear %1 selected columns",
      "ClearSelectedColsValues": "Clear %1 selected column values",
      "ClearSelectedColsStyles": "Clear %1 selected column styles",
      "ClearRow": "Clear the row",
      "ClearRowValues": "Clear the row values",
      "ClearRowStyles": "Clear the row styles",
      "ClearSelectedRows": "Clear %1 selected rows",
      "ClearSelectedRowsValues": "Clear %1 selected row values",
      "ClearSelectedRowsStyles": "Clear %1 selected row styles",
      "ClearAll": "Clear all cells",
      "ClearAllValues": "Clear all cell values",
      "ClearAllStyles": "Clear all cell styles",
      "SetCellAbsolute": "Set the cell reference absolute",
      "SetCellRelative": "Set the cell reference relative",
      "SwitchCellAbsolute": "Switch cell reference",
      "SwitchRowColAbsolute": "Switch cell reference",
      "ChooseRowInsert": "Insert row reference",
      "ChooseRowReplace": "Replace row reference",
      "ChooseRowReplaceAll": "Replace row reference",
      "ChooseColInsert": "Insert column reference",
      "ChooseColReplace": "Replace column reference",
      "ChooseColReplaceAll": "Replace column reference",
      "ChooseCellInsert": "Insert cell reference",
      "ChooseCellReplace": "Replace cell reference",
      "ChooseCellReplaceAll": "Replace cell reference",
      "CalcOn": "Enable calculations",
      "CalcOff": "Disable calculations",
      "ClearBorder": "Clear cell border",
      "ClearBorderTop": "Clear cell top border",
      "ClearBorderBottom": "Clear cell bottom border",
      "ClearBorderLeft": "Clear cell left border",
      "ClearBorderRight": "Clear cell right border",
      "SetBorder": "Set cell border",
      "SetBorderTop": "Set cell top border",
      "SetBorderBottom": "Set cell bottom border",
      "SetBorderLeft": "Set cell left border",
      "SetBorderRight": "Set cell right border",
      "SetBorderIn": "Set cell inner border",
      "SetBorderInTop": "Set cell inner top border",
      "SetBorderInBottom": "Set cell inner bottom border",
      "SetBorderInLeft": "Set cell inner left border",
      "SetBorderInRight": "Set cell inner right border",
      "ClearBorders": "Clear border in %1 cells",
      "ClearBordersOut": "Clear outer border in %1 cells",
      "ClearBordersIn": "Clear inner border in %1 cells",
      "ClearBordersTop": "Clear top border in %1 cells",
      "ClearBordersBottom": "Clear bottom border in %1 cells",
      "ClearBordersLeft": "Clear left border in %1 cells",
      "ClearBordersRight": "Clear right border in %1 cells",
      "SetBorders": "Set border in %1 cells",
      "SetBordersOut": "Set outer border in %1 cells",
      "SetBordersOutTop": "Set outer top border in %1 cells",
      "SetBordersOutBottom": "Set outer bottom border in %1 cells",
      "SetBordersOutLeft": "Set outer left border in %1 cells",
      "SetBordersOutRight": "Set outer right border in %1 cells",
      "SetBordersIn": "Set inner border in %1 cells",
      "SetBordersInTop": "Set inner top border in %1 cells",
      "SetBordersInBottom": "Set inner bottom border in %1 cells",
      "SetBordersInLeft": "Set inner left border in %1 cells",
      "SetBordersInRight": "Set inner right border in %1 cells",
      "ClearSelectedBorders": "Clear border in %1 selected cells",
      "ClearSelectedBordersTop": "Clear top border in %1 selected cells",
      "ClearSelectedBordersBottom": "Clear bottom border in %1 selected cells",
      "ClearSelectedBordersLeft": "Clear left border in %1 selected cells",
      "ClearSelectedBordersRight": "Clear right border in %1 selected cells",
      "ClearSelectedBordersOut": "Clear outer border in %1 selected cells",
      "ClearSelectedBordersIn": "Clear inner border in %1 selected cells",
      "SetSelectedBorders": "Set border in %1 selected cells",
      "SetSelectedBordersOut": "Set outer border in %1 selected cells",
      "SetSelectedBordersOutTop": "Set outer top border in %1 selected cells",
      "SetSelectedBordersOutBottom": "Set outer bottom border in %1 selected cells",
      "SetSelectedBordersOutLeft": "Set outer left border in %1 selected cells",
      "SetSelectedBordersOutRight": "Set outer right border in %1 selected cells",
      "SetSelectedBordersIn": "Set inner border in %1 selected cells",
      "SetSelectedBordersInTop": "Set inner top border in %1 selected cells",
      "SetSelectedBordersInBottom": "Set inner bottom border in %1 selected cells",
      "SetSelectedBordersInLeft": "Set inner left border in %1 selected cells",
      "SetSelectedBordersInRight": "Set inner right border in %1 selected cells",
      "ChooseBorderEdge": "Choose cell border edge",
      "ChooseBorderEdgeItems": "%1",
      "ChooseBorderStyle": "Choose cell border style",
      "ChooseBorderStyleItems": "%1",
      "ChooseBorderColor": "Choose cell border color",
      "ChooseBorderColorItems": "<span style='background:%2;padding-left:15px;margin-right:5px;border-radius:50px;height:12px;font-size:12px;height:10px;'></span>%1",
      "ChooseBorder": "Choose cell border",
      "ChooseBordersEdge": "Choose border edge in %1 cells",
      "ChooseBordersStyle": "Choose border style in %1 cells",
      "ChooseBordersColor": "Choose border color in %1 cells",
      "ChooseBorders": "Choose border in %1 cells",
      "ChooseSelectedBordersEdge": "Choose border edge in %1 selected cells",
      "ChooseSelectedBordersStyle": "Choose border style in %1 selected cells",
      "ChooseSelectedBordersColor": "Choose border color in %1 selected cells",
      "ChooseSelectedBorders": "Choose border in %1 selected cells",
      "RotateLeft": "Rotate cell left",
      "RotateRight": "Rotate cell right",
      "NoRotate": "Clear cell rotation",
      "VAlignTop": "Align cell value top",
      "VAlignBottom": "Align cell value bottom",
      "VAlignMiddle": "Center cell value vertically",
      "NoVAlign": "Clear cell vertical alignment",
      "AlignLeft": "Align cell value left",
      "AlignRight": "Align cell value right",
      "AlignCenter": "Center cell value horizontally",
      "NoAlign": "Clear cell horizontal alignment",
      "WrapOn": "Wrap cell value",
      "WrapOff": "Do not wrap cell value",
      "NoWrap": "Use default cell wrapping",
      "NoTextStyle": "Use cell default text style",
      "BoldOn": "Use bold font in cell",
      "BoldOff": "Do not use bold font in cell",
      "ItalicOn": "Use italic font in cell",
      "ItalicOff": "Do not use italic font in cell",
      "UnderlineOn": "Underline cell text",
      "UnderlineOff": "Do not underline cell text",
      "StrikeOn": "Strike cell text",
      "StrikeOff": "Do not strike cell text",
      "OverlineOn": "Overline cell text",
      "OverlineOff": "Do not overline cell text",
      "SmallCapsOn": "Use small caps in cell",
      "SmallCapsOff": "Do not use small caps in cell",
      "SetTextColor": "Set cell text color",
      "NoTextColor": "Clear cell text color",
      "ChooseTextColor": "Choose cell text color",
      "ChooseTextColorItems": "<span style='background:%2;padding-left:15px;margin-right:5px;border-radius:50px;height:12px;font-size:12px;'></span>%1",
      "SetColor": "Set cell background color",
      "NoColor": "Clear cell background color",
      "ChooseColor": "Choose cell background color",
      "ChooseColorItems": "<span style='background:%2;padding-left:15px;margin-right:5px;border-radius:50px;height:12px;font-size:12px;'></span>%1",
      "SetTextSize": "Set cell font size",
      "NoTextSize": "Set default cell font size",
      "ChooseTextSize": "Choose cell font size",
      "ChooseTextSizeItems": "%1",
      "SetTextFont": "Set cell font name",
      "NoTextFont": "Set default cell font name",
      "ChooseTextFont": "Choose cell font name",
      "ChooseTextFontItems": "%1",
      "SetTextShadowColor": "Set cell text shadow color",
      "NoTextShadowColor": "Clear cell text shadow color",
      "ChooseTextShadowColor": "Choose cell text shadow color",
      "ChooseTextShadowColorItems": "<span style='background:%2;padding-left:15px;margin-right:5px;border-radius:50px;height:12px;font-size:12px;'></span>%1",
      "SetTextShadow": "Set cell text shadow",
      "NoTextShadow": "Clear cell text shadow",
      "ChooseTextShadow": "Choose cell text shadow",
      "ChooseTextShadowItems": "%1",
      "RotateLeftCells": "Rotate %1 cells left",
      "RotateRightCells": "Rotate %1 cells right",
      "NoRotateCells": "Clear rotation in %1 cells",
      "VAlignTopCells": "Align value top in %1 cells",
      "VAlignBottomCells": "Align value bottom in %1 cells",
      "VAlignMiddleCells": "Center value vertically in %1 cells",
      "NoVAlignCells": "Clear vertical alignment in %1 cells",
      "AlignLeftCells": "Align value left in %1 cells",
      "AlignRightCells": "Align value right in %1 cells",
      "AlignCenterCells": "Center value horizontally in %1 cells",
      "NoAlignCells": "Clear horizontal alignment in %1 cells",
      "WrapOnCells": "Wrap value in %1 cells",
      "WrapOffCells": "Do not wrap value in %1 cells",
      "NoWrapCells": "Use default wrapping in %1 cells",
      "NoTextStyleCells": "Use default text style in %1 cells",
      "BoldOnCells": "Use bold font in %1 cells",
      "BoldOffCells": "Do not use bold font in %1 cells",
      "ItalicOnCells": "Use italic font in %1 cells",
      "ItalicOffCells": "Do not use italic font in %1 cells",
      "UnderlineOnCells": "Underline text in %1 cells",
      "UnderlineOffCells": "Do not underline text in %1 cells",
      "StrikeOnCells": "Strike text in %1 cells",
      "StrikeOffCells": "Do not strike text in %1 cells",
      "OverlineOnCells": "Overline text in %1 cells",
      "OverlineOffCells": "Do not overline text in %1 cells",
      "SmallCapsOnCells": "Use small caps in %1 cells",
      "SmallCapsOffCells": "Do not use small caps in %1 cells",
      "SetTextColorCells": "Set text color in %1 cells",
      "NoTextColorCells": "Clear text color in %1 cells",
      "ChooseTextColorCells": "Choose text color in %1 cells",
      "ChooseTextColorItemsCells": "%1",
      "SetColorCells": "Set background color in %1 cells",
      "NoColorCells": "Clear background color in %1 cells",
      "ChooseColorCells": "Choose background color in %1 cells",
      "ChooseColorItemsCells": "%1",
      "SetTextSizeCells": "Set font size in %1 cells",
      "NoTextSizeCells": "Set default font size in %1 cells",
      "ChooseTextSizeCells": "Choose font size in %1 cells",
      "ChooseTextSizeItemsCells": "%1",
      "SetTextFontCells": "Set font name in %1 cells",
      "NoTextFontCells": "Set default font name in %1 cells",
      "ChooseTextFontCells": "Choose font name in %1 cells",
      "ChooseTextFontItemsCells": "%1",
      "SetTextShadowColorCells": "Set text shadow color in %1 cells",
      "NoTextShadowColorCells": "Clear text shadow color in %1 cells",
      "ChooseTextShadowColorCells": "Choose text shadow color in %1 cells",
      "ChooseTextShadowColorItemsCells": "%1",
      "SetTextShadowCells": "Set text shadow in %1 cells",
      "NoTextShadowCells": "Clear text shadow in %1 cells",
      "ChooseTextShadowCells": "Choose text shadow in %1 cells",
      "ChooseTextShadowItemsCells": "%1",
      "RotateLeftSelected": "Rotate %1 selected cells left",
      "RotateRightSelected": "Rotate %1 selected cells right",
      "NoRotateSelected": "Clear rotation in %1 selected cells",
      "VAlignTopSelected": "Align value top in %1 selected cells",
      "VAlignBottomSelected": "Align value bottom in %1 selected cells",
      "VAlignMiddleSelected": "Center value vertically in %1 selected cells",
      "NoVAlignSelected": "Clear vertical alignment in %1 selected cells",
      "AlignLeftSelected": "Align value left in %1 selected cells",
      "AlignRightSelected": "Align value right in %1 selected cells",
      "AlignCenterSelected": "Center value horizontally in %1 selected cells",
      "NoAlignSelected": "Clear horizontal alignment in %1 selected cells",
      "WrapOnSelected": "Wrap value in %1 selected cells",
      "WrapOffSelected": "Do not wrap value in %1 selected cells",
      "NoWrapSelected": "Use default wrapping in %1 selected cells",
      "NoTextStyleSelected": "Use default text style in %1 selected cells",
      "BoldOnSelected": "Use bold font in %1 selected cells",
      "BoldOffSelected": "Do not use bold font in %1 selected cells",
      "ItalicOnSelected": "Use italic font in %1 selected cells",
      "ItalicOffSelected": "Do not use italic font in %1 selected cells",
      "UnderlineOnSelected": "Underline text in %1 selected cells",
      "UnderlineOffSelected": "Do not underline text in %1 selected cells",
      "StrikeOnSelected": "Strike text in %1 selected cells",
      "StrikeOffSelected": "Do not strike text in %1 selected cells",
      "OverlineOnSelected": "Overline text in %1 selected cells",
      "OverlineOffSelected": "Do not overline text in %1 selected cells",
      "SmallCapsOnSelected": "Use small caps in %1 selected cells",
      "SmallCapsOffSelected": "Do not use small caps in %1 selected cells",
      "SetTextColorSelected": "Set text color in %1 selected cells",
      "NoTextColorSelected": "Clear text color in %1 selected cells",
      "ChooseTextColorSelected": "Choose text color in %1 selected cells",
      "ChooseTextColorItemsSelected": "%1",
      "SetColorSelected": "Set background color in %1 selected cells",
      "NoColorSelected": "Clear background color in %1 selected cells",
      "ChooseColorSelected": "Choose background color in %1 selected cells",
      "ChooseColorItemsSelected": "%1",
      "SetTextSizeSelected": "Set font size in %1 selected cells",
      "NoTextSizeSelected": "Set default font size in %1 selected cells",
      "ChooseTextSizeSelected": "Choose font size in %1 selected cells",
      "ChooseTextSizeItemsSelected": "%1",
      "SetTextFontSelected": "Set font name in %1 selected cells",
      "NoTextFontSelected": "Set default font name in %1 selected cells",
      "ChooseTextFontSelected": "Choose font name in %1 selected cells",
      "ChooseTextFontItemsSelected": "%1",
      "SetTextShadowColorSelected": "Set text shadow color in %1 selected cells",
      "NoTextShadowColorSelected": "Clear text shadow color in %1 selected cells",
      "ChooseTextShadowColorSelected": "Choose text shadow color in %1 selected cells",
      "ChooseTextShadowColorItemsSelected": "%1",
      "SetTextShadowSelected": "Set text shadow in %1 selected cells",
      "NoTextShadowSelected": "Clear text shadow in %1 selected cells",
      "ChooseTextShadowSelected": "Choose text shadow in %1 selected cells",
      "ChooseTextShadowItemsSelected": "%1",
      "SetEditable": "Permit cell editing",
      "SetReadOnly": "Make cell read only",
      "SetPreview": "Make cell preview",
      "ClearEditable": "Clear cell edit restrictions",
      "SetEditableCells": "Permit editing in focused cells",
      "SetReadOnlyCells": "Make focused cells read only",
      "SetPreviewCells": "Make focused cells preview",
      "ClearEditableCells": "Clear edit restrictions in focused cells",
      "SetEditableSelected": "Permit editing in selected cells",
      "SetReadOnlySelected": "Make selected cells read only",
      "SetPreviewSelected": "Make selected cells preview",
      "ClearEditableSelected": "Clear edit restrictions in selected cells",
      "SetEditableRow": "Permit row editing",
      "SetReadOnlyRow": "Make row read only",
      "SetPreviewRow": "Make row preview",
      "ClearEditableRow": "Clear row edit restrictions",
      "SetEditableSelectedRows": "Permit editing in selected rows",
      "SetReadOnlySelectedRows": "Make selected rows read only",
      "SetPreviewSelectedRows": "Make selected rows preview",
      "ClearEditableSelectedRows": "Clear edit restrictions in selected rows",
      "SetEditableCol": "Permit column editing",
      "SetReadOnlyCol": "Make column read only",
      "SetPreviewCol": "Make column preview",
      "ClearEditableCol": "Clear column edit restrictions",
      "SetEditableSelectedCols": "Permit editing in selected columns",
      "SetReadOnlySelectedCols": "Make selected columns read only",
      "SetPreviewSelectedCols": "Make selected columns preview",
      "ClearEditableSelectedCols": "Clear edit restrictions in selected columns",
      "ChooseFormat": "Choose cell format",
      "ChooseFormatCells": "Choose format in %1 cells",
      "ChooseFormatSelected": "Choose format in %1 selected cells",
      "SetFormat": "Set cell format",
      "SetFormatCells": "Set format in %1 cells",
      "SetFormatSelected": "Set format in %1 selected cells",
      "SpreadContextDisplay": "Display",
      "SpreadContextManipulate": "Manipulate",
      "SpreadContextEdit": "Edit",
      "SpreadContextClear": "Clear",
      "SpreadContextFormat": "Format",
      "SpreadContextSpan": "Span",
      "SpreadContextBorder": "Border",
      "SpreadContextStyle": "Style",
      "SpreadContextFont": "Font",
      "SpreadContextColor": "Color",
      "SpreadContextShadow": "Shadow",
      "SpreadContextAlign": "Align",
      "SpreadContextDirection": "Direction",
      "Copy": "Copy to clipboard",
      "Exclude": "Cut to clipboard",
      "Import": "Import local XLSX file to the sheet"
    },
    "MenuCfg": {
      "Caption": "IBSheet settings",
      "ShowDeleted": "Show deleted rows (in red)",
      "AutoSort": "Auto sort rows after change",
      "AutoUpdate": "Auto update changes to server",
      "MouseHover": "Mouse hover",
      "Hover1": "None",
      "Hover2": "Cell only",
      "Hover3": "Row",
      "Hover4": "Row and column",
      "ShowDrag": "Show dragged object",
      "ShowPanel": "Show row's left panel",
      "ShowPager": "Show pager",
      "ShowAllPages": "Show all pages",
      "CheckUpdates": "Check for updates on server every",
      "CheckUpdates0": "Off",
      "CheckUpdates2": "2 seconds",
      "CheckUpdates5": "5 seconds",
      "CheckUpdates10": "10 seconds",
      "CheckUpdates30": "30 seconds",
      "CheckUpdates60": "1 minute",
      "CheckUpdates120": "2 minutes",
      "CheckUpdates300": "5 minutes",
      "CheckUpdates600": "10 minutes",
      "SortClick": "Sorting click",
      "SortClick1": "Simple, no icons",
      "SortClick2": "Simple",
      "SortClick3": "Directional, icons only",
      "SortClick4": "Directional",
      "Correct1": "manually",
      "Correct2": "auto",
      "Correct3": "ask",
      "Check1": "permit",
      "Check2": "restrict",
      "Check3": "ask",
      "Check4": "alert",
      "Scrollbars": "Scrollbars type",
      "Scrollbars1": "None",
      "Scrollbars2": "Thin",
      "Scrollbars3": "Standard",
      "Scrollbars4": "Wide",
      "Scroll": "Scroll by",
      "Scroll1": "Only scrollbars",
      "Scroll2": "One finger (no move)",
      "Scroll3": "Two fingers (no zoom)",
      "Scroll4": "Three fingers (no drag)",
      "ReversedTree": "Reversed tree (like in MS Excel)",
      "ShowButtons": "Show buttons",
      "ShowButtons1": "Icons only",
      "ShowButtons2": "Texts only",
      "ShowButtons3": "Icons and texts",
      "CfgCaption": "Basic settings",
      "Animations": "Animate actions",
      "ShrinkStyle": "Shrink style in small space",
      "ShrinkStyle1": "Never",
      "ShrinkStyle2": "Always",
      "ShrinkStyle3": "Ask",
      "CompatibleStyles": "Compatible",
      "Style": "Style",
      "StyleSize": "Style size",
      "Scale": "Scale",
      "StylesCaption": "-",
      "ShowBorder": "Show cell default border",
      "FormulaTip": "Show formula as cell tip",
      "ShrinkStyle4": "Minimal",
      "EditEnumSeparator": "\t"
    },
    "MenuColumns": {
      "Caption": "Choose items",
      "ColsCaption": "Choose columns",
      "PagersCaption": "Choose pagers",
      "SpaceCaption": "Choose toolbars",
      "RowsCaption": "Choose rows",
      "FirstPrintHeadCaption": "Select header printing"
    },
    "MenuPrint": {
      "Caption": "Print options",
      "PrintOptions": "Print options",
      "PDFName": "File name",
      "PDFFormat": "",
      "PDFFormats": "PDF",
      "PrintRows": "Maximum rows per page",
      "PrintPageRoot": "Start page always by root row",
      "PDFText": "",
      "PDFTexts": "Image only,Selectable text,Embedded fonts",
      "PrintExpanded": "Print all rows expanded",
      "PrintFiltered": "Print all rows (unfiltered)",
      "PrintSelectedRows": "Print selected rows only",
      "PrintSelectedCols": "Print selected columns only",
      "PrintSelectedCells": "Print selected cells only",
      "PrintVisible": "Print visible only",
      "PrintPageOptions": "Page size<div style='font-size:10px;color:#EA4647;white-space:normal;'>Set&nbsp;the&nbsp;same&nbsp;values&nbsp;also in&nbsp;the&nbsp;browser&nbsp;print&nbsp;dialog!</div>",
      "PDFPageOptions": "Page size",
      "PrintPageSize": "Page size",
      "PrintPageSizes": "Auto,26000,26000,Letter (8.5x11in),215.9,279.4,Legal (8.5x14in),215.9,355.6,Ledger (11x17in),279.4,431.8,A0 - (841x1189),841,1189,A1 - (594x841),594,841,A2 - (420x594),420,594,A3 - (297x420),297,420,A4 - (210x297),210,297,A5 - (148x210),148,210,A6 - (105x148),105,148,A7 - (74x105),74,105,A8 - (52x74),52,74,A9 - (37x52),37,52,B0 - (1000x1414),1000,1414,B1 - (707x1000),707,1000,B2 - (500x707),500,707,B3 - (353x500),353,500,B4 - (250x353),250,353,B5 - (176x250),176,250",
      "PrintPageOrientation": "Page orientation",
      "PrintPageOrientations": "Portrait,Landscape",
      "PrintMarginWidth": "Page margin width (mm)",
      "PrintMarginHeight": "Page margin height (mm)",
      "PDFFitPage": "Fit to one page",
      "PDFFitPages": "None,Width,Height,Single page",
      "PrintDPI": "DPI (PPI) ratio",
      "PrintWidth": "Page width (px)",
      "PrintHeight": "Page height (px)",
      "PrintResults": "Print size is %d x %d pages",
      "FirstPrintHead": "Header output only on the first print"
    },
    "MenuExport": {
      "Caption": "Export options",
      "ExportOptions": "Export options",
      "ExportName": "File name",
      "ExportFormat": "File format",
      "ExportFormats": "XLSX,XLS,CSV",
      "ExportExpanded": "Export all rows expanded",
      "ExportFiltered": "Export all rows (unfiltered)",
      "ExportOutline": "Export tree in Excel outline",
      "ExportIndent": "Indent tree in main column",
      "ExportSelectedRows": "Export selected rows only",
      "ExportSelectedCols": "Export selected columns only",
      "ExportSelectedCells": "Export selected cells only",
      "ExportFixedRows": "Freeze (anchor) head rows",
      "ExportFixedCols": "Freeze (anchor) left columns",
      "ExportEmpty": "Export also empty cells"
    },
    "MenuFilter": {
      "F0": "Off",
      "F1": "Equal",
      "F2": "Not equal",
      "F3": "Less than",
      "F4": "Less than or equal",
      "F5": "Greater than",
      "F6": "Greater than or equal",
      "F7": "Begins with",
      "F8": "Does not begin with",
      "F9": "Ends with",
      "F10": "Does not end with",
      "F11": "Contains",
      "F12": "Does not contain",
      "F13": "Top 10",
      "F14": "Has value",
      "F15": "No value"
    },
    "Format": {
      "YearName": "",
      "LongDayNames": "Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday",
      "ShortDayNames": "Sun,Mon,Tue,Wed,Thu,Fri,Sat",
      "Day2CharNames": "Su,Mo,Tu,We,Th,Fr,Sa",
      "Day1CharNames": "S,M,T,W,T,F,S",
      "LongMonthNames": "January,February,March,April,May,June,July,August,September,October,November,December",
      "LongMonthNames2": "January,February,March,April,May,June,July,August,September,October,November,December",
      "ShortMonthNames": "Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec",
      "DayNumbers": "1st,2nd,3rd,4th,5th,6th,7th,8th,9th,10th,11th,12th,13th,14th,15th,16th,17th,18th,19th,20th,21st,22nd,23rd,24th,25th,26th,27th,28th,29th,30th,31st",
      "Quarters": "I,II,III,IV",
      "Halves": "I,II",
      "DateSeparator": "/",
      "InputDateSeparators": "/.-",
      "TimeSeparator": ":",
      "InputTimeSeparators": ":",
      "AMDesignator": "AM",
      "PMDesignator": "PM",
      "FirstWeekDay": "0",
      "FirstWeekYearDay": "0",
      "NaD": "NaN",
      "Hirji": "0",
      "GMT": "0",
      "m": "MM/dd",
      "d": "MM/dd/yyyy",
      "h": "MM/dd/yyyy H:mm",
      "H": "MM/dd/yyyy H:mm:ss",
      "j": "MM/dd/yyyy h:mm tt",
      "J": "MM/dd/yyyy h:mm:ss tt",
      "y": "M/yyyy",
      "t": "HH:mm",
      "T": "HH:mm:ss",
      "i": "h:mm tt",
      "I": "h:mm:ss tt",
      "M": "dddddd MMMM",
      "D": "dddddd MMMM yyyy",
      "l": "dddddd MMMM yyyy H:mm",
      "L": "dddddd MMMM yyyy H:mm:ss",
      "k": "dddddd MMMM yyyy h:mm tt",
      "K": "dddddd MMMM yyyy h:mm:ss tt",
      "Y": "MMMMMMM yyyy",
      "g": "MM/dd/yyyy HH:mm",
      "G": "MM/dd/yyyy HH:mm:ss",
      "ValueSeparator": ";",
      "ValueSeparatorHtml": "; ",
      "RangeSeparator": "~",
      "RangeSeparatorHtml": " ~ ",
      "RepeatSeparator": "#",
      "CountSeparator": "*",
      "AddSeparator": "+",
      "DecimalSeparator": ".",
      "DecimalAdjust": "round",
      "InputDecimalSeparators": ".,",
      "GroupSeparator": ",",
      "InputGroupSeparators": "",
      "Percent": "%",
      "NaN": "NaN",
      "ValueAndSeparator": ","
    },
    "Debug": {
      "CannotLoadSourceData": "Cannot load IBSheet source data",
      "NoDataSource": "No data source defined",
      "NoMainTag": "No Main Tag defined",
      "CannotLoadCSS": "Cannot load IBSheet CSS style from ",
      "NoCSS": "No CSS style and url defined by $1",
      "Parsing": "Parsing",
      "Running": "Running",
      "Loading": "Loading",
      "Missing": "Missing",
      "Downloading": "Downloading",
      "Attaching": "Attaching",
      "Reloading": "Reloading",
      "Saving": "Saving",
      "Filtering": "Filtering",
      "Grouping": "Grouping",
      "Updating": "Updating",
      "Rendering": "Rendering",
      "Creating": "Creating",
      "Printing": "Printing",
      "Searching": "Searching",
      "Clearing": "Clearing",
      "Sorting": "Sorting",
      "Calculating": "Calculating",
      "FailedWithErr": "failed with error",
      "FailedWithException": "failed with exception",
      "NoDefineIO": "No $1.$2 defined",
      "CannotAccess": "Cannot access the",
      "ExceptionRaise": "exception raised:",
      "ExceptionMsg": "exception message:",
      "ErrMsg": "Error message:",
      "TimeOut": "Timeout ($1 sec) expired when accessing the",
      "FailedCommunication": "communication failed",
      "ServerCommunicationErr": "Error in communication with server from",
      "EmptyData": "Empty xml",
      "DataReceive": "received from server from",
      "WrongSyntax": "Wrong syntax in",
      "ConvertFormula": "formula was converted to",
      "WrongEditMask": "Wrong edit mask in cell",
      "WrongResultMask": "Wrong result mask in cell",
      "ResErr": "Error result",
      "FromDataSource": " from data source ",
      "Data": " data ",
      "TagName": "tag named",
      "InTagName": "in tag named",
      "FromTagName": "from tag named",
      "NoUploadTag": "Upload.Tag is set to \"Debug\", so the data are not uploaded, just printed here",
      "FailedReloadBody": "body failed with result",
      "NoFormTag": "Cannot submit data to server, no <form> tag found",
      "IncorrectJSONData": "Incorrect JSON data from $1, error returned: ",
      "NoFoundSheetData": "No tag <Sheet> found in data from $1",
      "TooManyData": "Too long XML data in: ",
      "InvaildDataStructure": "Invalid structure in XML data: ",
      "InvaildDataNodeStructure": "Invalid node structure in XML data: ",
      "InvaildDataNodeStructureWithParse": "The invalid XML node structure was parsed to: ",
      "MissingQuartation": "Missing quotes in attribute value in XML data: ",
      "MissingClose": "Missing closing tag in XML data: ",
      "MissingEnclosingData": "Missing enclosing tag </Sheet> in the root level in XML data from $1",
      "DownCell": "Downloaded cell",
      "FailedDownCell": "Failed downloading cell",
      "CellAction": "Cell action",
      "Action": "Action",
      "Failed": "failed",
      "GlobalEventHandler": "Global event handler",
      "ForEvent": "for event",
      "CallAction": "for called by DoAction()",
      "MissingCustomEndEdit": "Defined onCustomStartEdit, but missing onCustomEndEdit event",
      "UnknownAttribute": "Unknown attribute",
      "UnknownProperty": "Unknown property",
      "UnknownColumn": "Unknown column",
      "UnknownType": "Unknown Type",
      "UnknownStyle": "Unknown style",
      "UnknownFlag": "Unknown flag",
      "UnknownTag": "Unknown tag",
      "UnknownValueFor": "Unknown value for",
      "UnknownRootTag": "Unknown root tag",
      "UnknownDefault": "Unknown default",
      "InEventAction": "in event action",
      "MissingModule": "failed because of missing",
      "ErrFilter": "Error in Filter: ",
      "InGroupingCancel": "in grouping, grouping cancelled",
      "MissingLayoutData": "Missing layout data for detail sheet in cell",
      "MissingOnContextMenuEvent": "No onContextMenu event defined for custom popup menu item",
      "WrongMergeCell": "Wrong cell Merge attribute used, Merge must be set as list of column names, comma separated",
      "InCell": "in cell",
      "NoDefRow": "Not defined default row",
      "CannotAddRow": ", cannot add new row to this sheet",
      "FailedLayoutUpdate": "Update layout failed!",
      "FailedLayoutUpdateClearLimitScroll": "Update layout failed, clearing LimitScroll!",
      "ConflictObject2": "JavaScript global name conflict! You should rename your objects! Some script overwrote IBSheet global objects: ",
      "ConflictObject1": "JavaScript global name conflict! You should rename your objects! IBSheet overwrote global objects: ",
      "NoDataSourceInSheet": "Set at least one data source attribute in <ibsheet>:",
      "DataSourceList": " Data.Url, Data.Tag, Data.Data, Layout.Url, Layout.Tag, Layout.Data",
      "NoMainTagRequired": "Main Tag is a parent tag where IBSheet will be shown",
      "MissingHeight": "Missing row height definition for iPad, maybe loaded old or incorrect main.css?",
      "EmptyStylePrefix": "Empty style prefix ",
      "CompatibleTreeDynSpan": "Dynamic spanning in tree is incompatible with IE8/9 strict mode ! Switch IE to quirks mode or IE7 compatibility mode",
      "MissingJSZip": "No archivation method available! Include JSZip or JSZip library.",
      "LimitCookie": "Cookie is longer than 4096 bytes, it can be saved incorrectly. Use PersistentCfg to solve this problem.",
      "IncorrectFormat2": "Incorrect condition execution in format ",
      "IncorrectFormat1": "Incorrect condition in format ",
      "InvaildJSONParse": "Invalid JSON$1, the parse error: ",
      "NoPage": "No page (Body) defined in input XML",
      "CannotUpdateRow": "Cannot update row with id='",
      "NotFoundRow": "', the row not found",
      "ConflictsCfgToAPI1": "Wrong custom attribute name, probably in Cfg, conflicts with method name: ",
      "UnexpectedTag": "Unexpected child tag",
      "OfTag": "of tag",
      "ResServerData": "data in server response",
      "DupFixedRow1": "contain new fixed row '$1' definition, it is not possible to add new fixed rows by this response",
      "ReloadBodyDataIn": "ReloadBody: data in",
      "DupFixedRow2": "server response contain new fixed row '$1' definition, it is not possible to add new fixed rows via ReloadBody, define the fixed row in Layout $2 only",
      "DupColumn1": "contain new column '$1' definition, it is not possible to add new columns by this response",
      "DupColumn2": "server response contain new column '$1' definition, it is not possible to add new columns via ReloadBody, define columns in Layout $2 only",
      "DupSpaceRow1": "contain Space row(s) definition, it is not possible to update solid Space rows directly, use Changes instead",
      "DupSpaceRow2": "server response contain Space row(s) definition, it is not possible to update Space rows via ReloadBody, define the Space rows in Layout $1 only",
      "DupBodyRow": "contain <Body> rows definition, it is not possible to update rows directly, use Changes instead",
      "FoundDefRow": "Found default row Def without",
      "IgnoreRow": "attribute set, the row is ignored.",
      "FoundDefCol": "Found default column Def without",
      "IgnoreCol": "attribute set, the column is ignored.",
      "FoundCalendar": "Found calendar <E> without",
      "FoundResource": "Found resource <R> without",
      "IgnoreResource": "attribute set, the resource is ignored.",
      "ConflictsCfgToAPI2": "Wrong custom attribute name in Cfg, conflicts with method name: ",
      "UnknownFilterId": "Found Filter row with unknown id ",
      "ContainsMedia": "<Media> contains $1",
      "DefRow": "default row",
      "Default": "default",
      "Row": "row",
      "Column": "column",
      "ColumnName": "Column name ",
      "CellName": "Cell name ",
      "Without": "without",
      "Fixed": "fixed",
      "SetAttribute": "attribute set.",
      "InDataFrom": "in $1 data from",
      "UnknownAttributeIBSheet": "Tag <ibsheet> contains unknown attribute",
      "ObsoleteAttribute": "Obsolete attribute",
      "Use": "use",
      "Instead": "instead",
      "In": "in $1 ",
      "Page": "page",
      "ObsoleteFilterTypeInFilterRow": "Obsolete attribute FilterType in Filter row, use ",
      "ObsoleteAttributeRowsHeader": "Obsolete attribute Rows in Header row, define more Header rows instead",
      "ConflictColAttr": " conflicts with row attribute or browser attribute name",
      "ConflictCellAttr": " conflicts with row or cell or browser attribute name",
      "ConflictSpaceCellsAttr": " in space row $1 conflicts with row or another cell or browser attribute name",
      "IncorrectDefaults": "Incorrect Defaults in ",
      "IncorrectEnumMenu": "Incorrect EnumMenu in ",
      "NotAvailable": "IBSheet $1 not available",
      "ExportModule": "Export module",
      "NotSupportedDirectSaving": "Direct saving not supported and missing server Export.Url/Script",
      "InvalidColor": "Invalid color: ",
      "NoReturnPage": "No rows returned for page ",
      "DeletedPage": ", page is deleted.",
      "OfAttr": " attribute - ",
      "WarnRowHeightTooManyRows": "Updating row heights is very slow, because too many rows are higher than their or default row height ($1px), try to set the rows' heights by Height attribute. Now the first five rows have these heights: ",
      "DupRowId": "Duplicate row id ",
      "NoChild": "No children returned for ",
      "TooHeightBody1": "The sheet body is too high (",
      "TooHeightBody2": "), you should use PageLengthDiv to reduce it to avoid problems in IE8",
      "SheetId": "Sheet id",
      "SheetWithId": "Sheet with id",
      "CannotNumber": "cannot be set to number",
      "AlreadySameValue": "already exists or collides with some internal name",
      "NoStyleList": "No style list defined in Cfg ",
      "SwitchedCSS": "CSS switched to ",
      "SafeMode": "Safe mode",
      "AffectedExternalCSS": ", because IBSheet tag ($1) is affected by external CSS styles, you should remove this influence or set ",
      "CannotMaxWidthMaxHeight": "Cfg.$1 attribute cannot be used in this layout (e.g. floating or absolute parent without width in pixels), you should set fixed $2 for main <div> tag or set Cfg attribute No$3Scroll / Max$3Scroll. Now there is automatically set ",
      "AutoSetMaxWidthMaxHeightPixel1": "You should set $1 in ",
      "AutoSetMaxWidthMaxHeightPixel2": " for main <div> tag or set Cfg attribute $1. Now there is automatically set ",
      "AutoSetMaxWidthMaxHeight": "You should set $2 for main <div> tag or set Cfg attribute $1. Now there is automatically set $2 of the main <div> to ",
      "AutoSetNestedSheetVScroll": "Nested sheet should have set NoVScroll or MaxVScroll. Now there is automatically set ",
      "NotSupportedCanvas": "Canvas not available - image cannot be retrieved.",
      "ImgProcsErr": "Image processing error: ",
      "NotFoundImg": "Image not found: ",
      "ExportOptionsParseErr": "ExportOptions JSON parsing failed: ",
      "Error": "error",
      "GetTypeErr": "cell getType() error: ",
      "FailedExportFormat": "Apply format failed for str: ",
      "Format": "format",
      "FailedGetFile1": "Failed to get file ",
      "FailedGetFile2": " referenced in ",
      "LoadStyle": "Loading styles",
      "AJAXFor": "AJAX for ",
      "NoResSend": ": No request XML to send",
      "NoResSent": ": No request was finally sent because of setting Method=",
      "Children": "children",
      "From": "from",
      "DownloadingCell": "Downloading cell",
      "RenderedIn": "rendered in",
      "Downloaded": "Downloaded",
      "Update": "Update",
      "Color": "Color",
      "UpdateTree": "Update tree",
      "Calculate": "Calculate",
      "StartDebugger": "◑◑◑◑◑◑◑◑◑◑ Starting IBSheet ◐◐◐◐◐◐◐◐◐◐",
      "Called": "is called",
      "LoadingConfiguration": "Loading configuration is suppressed, default values from data are used",
      "NoCookieAttr": "No Cookie attribute set, default values from data are used",
      "LoadingPersistentStorage": "Loading configuration from persistent storage",
      "LoadingCookies": "Loading configuration from cookies",
      "LoadingCookieAttr": "Loading configuration from Cookie attribute",
      "NoSavedConfiguration": "No saved configuration is found, default values from data are used",
      "MovedIn": "moved in",
      "PagesIn": "pages in",
      "ChildPartsIn": "child parts in",
      "RenderCancel": "canceled, rendered",
      "RenderPageOnDemand": ", the rest of pages will be rendered on demand",
      "RenderedColPages": "IBSheet column pages rendered in ",
      "LoadingDataFrom": "Loading data from ",
      "ReloadingSheet": "Reloading sheet",
      "ReloadingBodyRows": "Reloading body rows due ",
      "ReloadingBodyFinished": "Reloading body finished in ",
      "SavingServerData": "Saving data to server to ",
      "DataSavedSuccess": "Data saved to server sucessfully in ",
      "ExportingSheet": "Exporting sheet",
      "ExportDataIn": "IBSheet generated export data in ",
      "FilteringRows": "Filtering rows",
      "GroupingRows": "Grouping rows",
      "FilteredIn": "IBSheet filtered in ",
      "GroupedIn": "IBSheet grouped in ",
      "UpdatedIn": "IBSheet updated in ",
      "SheetRenderedIn": "IBSheet rendered in ",
      "SortedIn": "IBSheet sorted in ",
      "UpdatingSheet": "Updating IBSheet",
      "RenderingPages": "Rendering pages",
      "UpdatedInData": "data updated: ",
      "UpdatedInCalculated": "calculated: ",
      "UpdatedInFilterAndSearch": "filtered and searched: ",
      "UpdatedInGrouped": "grouped: ",
      "UpdatedInSorted": "sorted: ",
      "UpdatedInPaged": "paged: ",
      "UpdatedInTree": "tree updated: ",
      "UpdatedInCol": "column widths calculated: ",
      "UpdatedInGantt": "Gantt created: ",
      "RenderedInCreateHTML": "create HTML: ",
      "RenderedInSetHTML": "set HTML: ",
      "RenderedInEvent": "update events: ",
      "RenderedInScrollbar": "update scrollbars: ",
      "RenderedInHeight": "update row heights: ",
      "SortedInPrepare": "prepare: ",
      "SortedInSort": "sort: ",
      "SortedInUpdate": "update: ",
      "CompletedFullTime1": "IBSheet completed in full time ",
      "CompletedFullTime2": " (without rendering pages)",
      "CompletedFullTime3": " (it is an exact time from starting and is affected by processing other sheets)",
      "RenderingChildPart": "Rendering child parts",
      "RenderedChildPartOnDemand": ", the rest of child parts will be rendered on demand",
      "SortingRows": "Sorting rows",
      "PrintingSheet": "Printing sheet",
      "RenderingSheet": "Rendering IBSheet",
      "UpdatingTree": "Updating tree",
      "CalculatingFormulas": "Calculating formulas",
      "CalculatingColumnWidths": "Calculating column widths",
      "CreatingPivotTable": "Creating pivot table",
      "ClearingSearch": "Clearing search action $1",
      "SearchCompletedIn": "Search completed in ",
      "StylesLoadedIn": "Styles loaded in ",
      "UpdatingLayoutAndValue": "Updating layout and values",
      "RenderedPage": "IBSheet pages rendered in ",
      "CreatePivot": "Pivot table created in ",
      "CompletedClearSearch": "Clearing search completed in ",
      "RenderedInChildPart": "IBSheet child parts rendered in ",
      "PrintReportIn": "IBSheet generated print report in ",
      "SearchingAction": "Searching in sheet, action $1",
      "LoadSuccessData": "IBSheet data loaded successfully in ",
      "NotFound": "Not found",
      "SkipLoading": "data source defined, skipped loading",
      "StyleCSSFile1": "Style prefix set to ",
      "StyleCSSFile2": ", used CSS file ",
      "CSSUse": "CSS uses ",
      "CalculatedColWidth": "Column widths calculated in ",
      "Results": "results: ",
      "NotImplement": "not implemented! src: ",
      "ShowDebug": "Debug shown",
      "CookieCfg": "Base settings: ",
      "ExpandAllCells": "Loaded collapsed / expanded state of all Expand cells",
      "VisibleExpandLevelRows": "Loaded Visible state of ExpandLevel rows",
      "VisibleAllRows": "Loaded Visible state of all rows",
      "ExpandAllRows": "Loaded collapsed / expanded state of all rows",
      "SelectedAllRows": "Loaded selected state of all rows",
      "NoSavedCusAttr": "No custom attributes saved",
      "LoadAllValues": "Loaded all cell values",
      "UnknownRowId": "Unknown row id=",
      "CusAttr": "Custom attributes: ",
      "IgnoreSortingSrtting": "Sorting setting is ignored: ",
      "NoSortingSrtting": "No sorting setting present",
      "IgnoreGroupingSrtting": "Grouping setting is ignored: ",
      "NoGroupingSrtting": "No grouping setting present. ",
      "IgnoreFilterSrtting": "Filter setting is ignored: ",
      "UsedFilterSrtting": "Filter setting is used: ",
      "NoFilterSrtting": "No filter setting present. ",
      "UsedSortingSetting": "Sorting setting is used: ",
      "UsedGroupingSetting": "Grouping setting is used: ",
      "SearchSetting": "Search settings: ",
      "PivotSetting": "Pivot settings: ",
      "ScrollPosition": "Scroll position: ",
      "SectionSizes": "Section sizes: ",
      "PrintOptions": "Print options: ",
      "ExportOptions": "Export options: ",
      "IgnoreColumnsSrtting": "Columns settings are ignored: ",
      "UsedColumnsSetting": "Columns settings are used: ",
      "ReceiveRes": "a response data got from server:",
      "SendRes": "a request data to send to server:",
      "SentRes": "a request sent to server:",
      "LoadDataLoaded": "a data loaded:",
      "LoadBonusAdded": "a bonus added:",
      "LoadXlsxSheetData": "a $1 loaded:",
      "UnknownSource": "Unknown data source",
      "XlsxSaveSheetErr": "Provided sheet data object is not valid, nowhere to save to.",
      "XlsxCreateSheetErr": "Cannot create sheet from provided data.",
      "XlsxNoData": "No data to convert!",
      "XlsxNoSheetList": "The provided dataset does not contain any sheets!",
      "XlsxDataSetWrong": "Sheet \"$1\" not available in the provided dataset!"
    },
    "Accessibility": {
      "Required": "Required",
      "AccCapInfo": "AccCapInfo",
      "IBSheet": "IBSheet",
      "AccDesc": "AccDesc",
      "AccAsc": "AccAsc",
      "BeginNotice": "This is The top of sheet %1.",
      "EndNotice": "This is The bottom of sheet %1.",
      "SectionLabel": "A %3 section of %2 in %1.",
      "SectionLeft": "Left",
      "SectionMid": "Middle",
      "SectionRight": "Right",
      "ColMove": "Column %1 moved to %2.",
      "ColResize": "The width of Column %1 increased by 5 pixels.",
      "ColSizeIncrease": "Increase",
      "ColSizeDecrease": "Decrease",
      "SortStatus": "The current sorting is in order %1.",
      "SortCancel": "Sorting canceled.",
      "SortAsc": "Ascending Order",
      "SortDesc": "Descending Order",
      "SortButton": "SortButton",
      "RadioGroup": "This is %1 radio group.",
      "FileEmpty": " The file does not exist.",
      "FileHas": " File",
      "FileDeleteGuide": "Enter the Delete key to delete the file, ",
      "FileAddGuide": "Enter or Space key to add the file.",
      "FileClear": "Deleted the files.",
      "IconClear": "Delete Icon",
      "IconDates": "Calendar Icon",
      "IconCheck": "Checked Icon",
      "IconUnCheck": "UnChecked Icon",
      "Filter": "%1 value is filtered by operator %2",
      "NoFilter": "Not filtered.",
      "IconAllCancel": "All Cancel",
      "Button": "Button",
      "Icon": "Icon",
      "Clear": "Clear",
      "Dates": "Calendar",
      "Check": "Checked",
      "UnCheck": "UnChecked",
      "TreeCollapse": "Collapsed %2 rows below %1.",
      "TreeExpand": "Expanded %2 rows below %1.",
      "IconTreeCollapse": "Tree Collapse Icon",
      "IconTreeExpand": "Tree Expand Icon",
      "TreeLevel": "%1 level of a tree.",
      "Menu": {
        "IconAllCancel": "All Cancel",
        "ItemSubMenu": "Press the space bar to open the submenu.",
        "ItemExpand": "Press the space bar to expand a folded row.",
        "ItemCollapse": "Press the space bar to fold open rows."
      },
      "Calendar": {
        "DayText": "Day Selected",
        "MonthText": "Selected",
        "YearText": "Year Selected",
        "SelDate": "This is currently selected.",
        "MoveNextMonth": "Move to previous month.",
        "MoveBeforeMonth": "Move to next month.",
        "MoveNext5Year": "Move to the next five years.",
        "MoveBefore5Year": "Move to the previous five years.",
        "MoveDayCheck": "Move to day selections.",
        "MoveMonthCheck": "Move to month selection."
      }
    },
    "BreakPoints": {
      "xs": 0,
      "sm": 576,
      "md": 768,
      "lg": 992,
      "xl": 1200,
      "xxl": 1400
    },
    "Dialog": {
      "Reset": "Reset",
      "DragHere": "Drag the target column here.",
      "HeaderTitle": "Header Title",
      "Show": "Show",
      "ColumnName": "Column Name",
      "SaveColumnInfo": "Save Col Infomation",
      "WillSave": "Do you want to save?",
      "EditDialog": "Edit Dialog",
      "Required": "Required",
      "Column": "Column",
      "Check": "CHECK",
      "Row": "ROW",
      "Col": "COLUMN",
      "Value": "VALUE",
      "FileUpload": "File Upload",
      "DataPreview": "Data Preview",
      "AddRow": "Add row",
      "DelRow": "Delete row",
      "ShowDetail": "Show Detail",
      "NeedCommonFile": "You will need the ibsheet-excel.js file.",
      "Caution": "Caution",
      "Caution1": "Error cases are given a yellow background. If there are errors, the data will not be loaded, so delete or edit them and click the 'Confirm' button.",
      "Caution2": "The upload dialog inserts data into each column in order from the first row of the Excel file.",
      "Caution3": "Only numbers can be entered into numeric columns.",
      "Caution4": "Only data from the list can be entered into the drop-down format column.",
      "Caution5": "You can enter 1 (selected) or 0 (deselected) in the checkbox type column.",
      "Caution6": "When uploading a text file, if a different delimiter is used than the one used in the file, the data may not appear correctly.",
      "Caution7": "There are currently error cases (yellow). Delete or modify the parts marked in yellow and click the 'Confirm' button.",
      "EnterSearchTerm": "Please enter your search term.",
      "EnterReplaceWord": "Please enter exactly what you want to change.",
      "SheetName": "Worksheet",
      "FileName": "Filename",
      "SetDelimiter": "Column Delimiter",
      "ChooseRow4Load": "Please select the rows you want to load.",
      "ChooseRow4Del": "Please select the row you want to delete.",
      "ChooseRow4Copy": "Please select the row you want to copy.",
      "ChooseRow4Move": "Please select the row to move.",
      "ReplacedWord": "%d items have been changed.",
      "NoFindData": "There is no data to find.",
      "ChartTypes": "Chart Type",
      "ChartData": "Chart Data",
      "Category": "Category",
      "Datas": "Datas",
      "SelectAll": "Select all",
      "AllColumn": "All Columns",
      "FindPrev": "Find prev",
      "FindNext": "Find next",
      "ChangeFunc": "Changes",
      "Replace": "Replace",
      "ReplaceAll": "Replace all",
      "CaseSensitive": "Case sensitive",
      "MatchEntireContent": "Match entire content",
      "IncludeCanNotEdit": "Includes no edits",
      "UnCheck": "Cancel selection",
      "SelectColumn": "Select column",
      "FindAll": "Find all",
      "PivotConfig": "Pivot function settings",
      "ShowPivot": "View pivot data",
      "PivotInfoMsg": "Drag and drop columns between the areas below.",
      "PivotColSearch": "Try searching for the column name.",
      "PivotColDelete": "Can remove it from the area by right-clicking on the column.",
      "Filter": "Filter",
      "PivotDupColumn": "%d Duplicate fields cannot be set.",
      "PivotLayOutUpdate": "Update right away",
      "PivotAll": "(All)",
      "PivotSelected": "(Selected)",
      "ShowOrgSheet": "View original data",
      "NormalColumn": "Columns (Normal)",
      "NumericColumn": "Columns (Numeric)",
      "BaseRow": "Base row",
      "BaseCol": "Base col",
      "PivotType": "Pivot type",
      "SavePivotConfig": "SavePivotSettings",
      "CreatePivot": "Create Pivot data",
      "NoPossibleColumn": "There are no possible target columns.",
      "NotComplete": "Pivot setup is not complete.",
      "DupDataWarn1": "The number of duplicate data in the column is exceeded. \n - Duplicate data count : %d\n - Recommended number of columns",
      "DupDataWarn2": "Exceeded number of duplicate data in row. \n - Duplicate data count : %d\n - Recommended number of rows",
      "WrongTypeWarn": "The type of the target column does not match.",
      "SortOrderTip": "The sorting criteria starts from the highest row.",
      "TargetTip": "Select the column to be sorted by.",
      "OrderTip": "Select whether to sort in ascending/descending order.",
      "NumberSortTip": "Select whether to sort data in numeric format.",
      "RawSortTip": "Choose whether to apply formatting to sorted data.\nWhen you select the 'Combo list order' menu, items are sorted in the order of the droplist.",
      "CaseSensitiveTip": "Choose whether to be case sensitive.\nIf checked, sorting is case sensitive.",
      "SortOrder": "Sort order",
      "BaseColumn": "Base Column",
      "Sort": "Sort",
      "SortEnum": "|Ascending|descending",
      "SortType": "Sort Type",
      "SortTypeEnum": "|String|Number",
      "UseFormat": "Format",
      "UseFormatEnum": "|Formatted Data|Raw Data|Combo list order",
      "SortDialog": "Sort Dialog",
      "AddCriteria": "Add",
      "DelCriteria": "Delete",
      "CopyCriteria": "Copy",
      "DelAllCriteria": "Delete All",
      "SortWarn1": "Please select the column you want to sort by.",
      "SortWarn2": "Duplicate columns exist.",
      "SortWarn3": "You can sort up to %d columns.",
      "UseTooltipDataLabel": "Use Tooltip/Datalabel",
      "Tooltip": "Tooltip",
      "DataLabel": "DataLabel",
      "Palette": "Palette",
      "LegendPos": "Layout Legend",
      "Hide": "Hide",
      "Top": "Top",
      "Bottom": "Bottom",
      "Left": "Left",
      "Right": "Right",
      "Use3d": "use 3d chart (Bar and pie charts only)",
      "Use": "3d Use",
      "UsePivot": "Pivot",
      "SwitchCategory": "Switch categories and series",
      "None": "None",
      "Select": "Select",
      "DownloadColSave": "Save settings",
      "Config": "Settings",
      "Download": "Download File",
      "FileDownload": "FileDownload",
      "PngDown": "Download Image File",
      "XlsxDown": "Download Excel File",
      "TextDown": "Download Text File",
      "Zoom": "Zoom In/Out",
      "TotalSum": "Grand Totals",
      "TotalRowSum": "Show grand totals for rows",
      "TotalColSum": "Show grand totals for columns",
      "ChartType": {
        "LineType": "Line type",
        "Line": "Line chart",
        "SPLine": "Spline chart",
        "StepLine": "Stepline chart",
        "ColumnType": "Column type",
        "Column": "Column chart",
        "StackedColumn": "Stacked column chart",
        "Stacked100Column": "100% stacked column chart",
        "BarType": "Bar type",
        "Bar": "bar chart",
        "StackedBar": "Stacked bar chart",
        "Stacked100Bar": "100% stacked bar chart",
        "PieType": "Pie type",
        "Pie": "Pie chart",
        "Doughnut": "doughnut chart",
        "AreaType": "Area type",
        "Area": "Area chart",
        "StackedArea": "Stacked area chart",
        "Stacked100Area": "100% stacked area chart",
        "ComplexChart": "complex type",
        "ColumnArea": "Column + Area chart",
        "ColumnLine": "Column + Line chart"
      },
      "ErrMsg": {
        "AtLeastOneData": "At least one data must be selected.",
        "InvalidSetting": "Cannot set this without a category.",
        "InvaildType": "The seletec type is invalid.",
        "MultipleSelected": "Chart creation is not possible when multiple areas are selected.",
        "NeedChart": "IBChart js file has not been loaded.",
        "NoSelectedRange": "No selected range exists.",
        "NoCategory": "There is no category in the selected area.",
        "NoData": "No data exists in the selected range."
      }
    },
    "DisplayMenu": {
      "ColumnVisibility": "Column Visibility",
      "ShowColumn": "Show Columns",
      "HideColumn": "Hide olumn",
      "ToggleSelectAll": "Select All / Deselect All",
      "AtLeastOneColumn": "At least one column must be visible."
    }
  }
};